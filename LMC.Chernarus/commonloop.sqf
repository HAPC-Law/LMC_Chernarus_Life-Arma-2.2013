...
if (_art == "clientloop") then
{
    ...
    while {true} do
    {
	if(vehicle player != player and !(vehicle player in INV_VehicleArray) and !(vehicle player in INV_ServerVclArray) and !(vehicle player in editorvehicles))then{deletevehicle (vehicle player)};
	if ((_Letzterbankmoney != leavemybankmoneyalone) and (leavemybankmoneyalone != StartGeld)) then
        {
            ["leavemybankmoneyalone", leavemybankmoneyalone] spawn ClientSaveVar;
            _Letzterbankmoney = leavemybankmoneyalone;
        };
	...
	_veh = vehicle player;
	if ((player distance getmarkerpos "policebase" < 250 or player distance getmarkerpos "policebase2" < 12 or player distance getmarkerpos "police_area_4" < 900) and isciv and !(_veh isKindOf "Air") and !(_veh isKindOf "Ship")) then
        {
            if (_veh != player) then
            {
	...
	if (_FuckOffMoneyHackers < 0) then {['moneyHackerFuckOff', 0] call INV_SetItemAmount; _FuckOffMoneyHackers = 0;};
	if (leavemybankmoneyalone > bank_limitonbank) then {leavemybankmoneyalone = bank_limitonbank;player groupChat localize "STRS_maxbank";};
        if (_FuckOffMoneyHackers > money_limitonbank) then {['moneyHackerFuckOff', money_limitonbank] call INV_SetItemAmount; player groupChat localize "STRS_maxmoney";};
	{
