...
if (_FuckOffMoneyHackers <= huren_cost)    exitWith {role groupChat localize "STRS_arbeiter_nomoney";};
['moneyHackerFuckOff', -(huren_cost)] call INV_AddInventoryItem;
...
while {true} do 
	...
	if((_hoename in wp1unitarray) or (_hoename in wp2unitarray) or (_hoename in wp3unitarray))then{timeinworkplace = timeinworkplace + hoemoneyprosekunde/base_workplace};
