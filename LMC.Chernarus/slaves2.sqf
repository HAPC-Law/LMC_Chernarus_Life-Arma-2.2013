...
if (_FuckOffMoneyHackers <= slave_cost) exitWith {role groupChat localize "STRS_slave_nomoney2"};

['moneyHackerFuckOff', -(slave_cost)] call INV_AddInventoryItem;
...
while {true} do 
	...
	for [{_i=0}, {_i < 60}, {_i=_i+1}] do 
		...
		if (player distance _slavename < 5 and _a1 == 0 and (call compile format ["arbeitergeld%1 > 0", _arbeiternummer])) then {_action = player addaction [localize "STRS_slave_moneyaction2", "slaves2.sqf",["moneyHackerFuckOff", _arbeiternummer]]; _a1 = 1;};															
...
if (_art == "moneyHackerFuckOff") exitWith 
...
if (_arbeitergeld > 0) then 
	...
	['moneyHackerFuckOff', _arbeitergeld] call INV_AddInventoryItem;
	call compile format ["arbeitergeld%1 = 0;", _arbeiternummer];	
