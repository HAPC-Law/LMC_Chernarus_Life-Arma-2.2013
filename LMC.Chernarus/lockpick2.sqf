private ["_closest","_closeVcl","_incarpark","_item"];
	...
	if (_closeVcl in INV_VehicleArray) then 
		...
		if ((random 100) < 50) then 
			...
			if (([player, (civarray + coparray), 40] call INV_Seen or _incarpark) and !iscop) then 
				...
				format [" [""StoleVcl"", %1, %3] spawn Isse_AddCrimeLogEntry; if(!(""vehicle theft(5mins)"" in %1_reason))then{%1_reason = %1_reason + [""vehicle theft(5mins)""]}; %1_wanted = 1; kopfgeld_%1 = kopfgeld_%1 + 20000; hint format[localize ""STRS_inventar_lockpick_gesehen"",%1, %3]; ", player, longrolenumber, _closeVcl] call broadcast;	
