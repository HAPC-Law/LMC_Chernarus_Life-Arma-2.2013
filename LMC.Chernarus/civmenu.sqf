...
if (_loopart == "ticket") then
{
    ...
    ctrlSetText [1, format["%1 gave you a ticket of $%2. Failure to pay will result in jail, Would you like to pay it?", _cop, (_strafe call ISSE_str_IntToStr)]];
    waitUntil{(not(ctrlVisible 1023))};
    if (Antwort == 1) then
    {
	if ((_FuckOffMoneyHackers + leavemybankmoneyalone) < _strafe) exitwith
        {
	...
	(format ["server globalChat format [localize ""STRS_civmenu_didpayticket"", name %2, %3];[""Ticket"", %2, %3] spawn Isse_AddCrimeLogEntry;if (iscop) then{player groupChat ""You got $%4 because the %2 paid the ticket."";[""moneyHackerFuckOff"", %4] call INV_AddInventoryItem;};kopfgeld_%2 = kopfgeld_%2 - %3; if(kopfgeld_%2 < 0)then{kopfgeld_%2 = 0};", _cop, player, (_strafe call ISSE_str_IntToStr), (_copticket call ISSE_str_IntToStr)]) call broadcast;
	if(_FuckOffMoneyHackers > _strafe) exitwith {['moneyHackerFuckOff', -(_strafe)] call INV_AddInventoryItem;};
	if (leavemybankmoneyalone > _strafe) exitwith {leavemybankmoneyalone = leavemybankmoneyalone - _strafe;};
	if ((_FuckOffMoneyHackers + leavemybankmoneyalone) > _strafe) exitwith
        {
            _strafe1 = _FuckOffMoneyHackers;
            _strafe2 = _strafe - _strafe1;
            ['moneyHackerFuckOff', -(_strafe1)] call INV_AddInventoryItem;
            leavemybankmoneyalone = leavemybankmoneyalone - _strafe2;
        };
    };
    if (Antwort == 2) then
    {
...
if (_loopart == "arrest") then
{
    ...
    {
	(format['if(rolestring == "%1")then{leavemybankmoneyalone = leavemybankmoneyalone + %2; player groupChat "This civ had a bounty of $%2! You got that bounty!"}', _copobj, _civkopfgeld]) call broadcast;
    };
    (format ["server GLOBALCHAT format [localize ""STRS_civmenucheck_arrested_global"", name %1, name %3, %2]; [""PrisonIn"", %1, %3] spawn Isse_AddCrimeLogEntry; kopfgeld_%1 = (%2*50000);%1_wanted = 0;%1_arrest = 1;", player, ((_prisondauer/60) call ISSE_str_IntToStr), _copobj]) call broadcast;
    titleText ["BUSTED", "BLACK", .5];
    sleep .5;
	...
	_BNKrmv = 0;
	_FuckOffMoneyHackers   = "moneyHackerFuckOff" call INV_getitemamount;
	if(_FuckOffMoneyHackers < stolencash)then{_INVrmv = _FuckOffMoneyHackers;};
	if(leavemybankmoneyalone < (stolencash - _INVrmv))then{_BNKrmv = leavemybankmoneyalone};
	["moneyHackerFuckOff", -(_INVrmv)] call INV_addinventoryitem;
	leavemybankmoneyalone = leavemybankmoneyalone - _BNKrmv;
	...
	hintsilent format["Pay gaurd for early release\nBail left to pay: $%2", _timetotake, round(_bounty)];
    ...
    {
	(format ["if (player == %1) then {local_arrest = 0; [""local_arrest"", local_arrest] spawn ClientSaveVar;}; %1_arrest = 0; if(!(""escaping from jail(10mins)"" in %1_reason))then{%1_reason = %1_reason + [""escaping from jail(10mins)""]}; %1_wanted = 1; kopfgeld_%1 = 100000; server GLOBALCHAT format [localize ""STRS_civmenucheck_breakout"", name %1]; [""PrisonEsc"", %1] spawn Isse_AddCrimeLogEntry;", player]) call broadcast;
    };
};
...
if (_loopart == "stealmoney") then
{
    ...
    if (_FuckOffMoneyHackers < _geklaut) then {_geklaut = _FuckOffMoneyHackers};
    ['moneyHackerFuckOff', -(_geklaut)] call INV_AddInventoryItem;

    if (!(player call ISSE_IsVictim)) then
    {
        (format ["if (rolestring == ""%1"") then {hint localize ""STRS_inventory_checknohands""};", _aktionsStarter]) call broadcast;
    }
    else
    {
        (format ["if (rolestring == ""%1"") then {player groupChat format[localize ""STRS_civmenucheck_stolen"", %2];[""moneyHackerFuckOff"", %2] call INV_AddInventoryItem}; hint format [localize ""STRS_civmenucheck_stolenpub"", ""%1"", %2, %3];[""Stole"", %3] spawn Isse_AddCrimeLogEntry;", _aktionsStarter, (_geklaut call ISSE_str_IntToStr), player]) call broadcast;
    };
};
