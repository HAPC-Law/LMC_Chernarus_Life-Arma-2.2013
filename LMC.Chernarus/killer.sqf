...

{
	...
	if (iscop or ("bounty" call INV_HasLicense and _civkopfgeld > 20000)) then 				//Ist der Mörder ein Cop oder ein Kopfgeldjäger
	{
		...
		(format['if (player == %3)then{leavemybankmoneyalone = leavemybankmoneyalone + %2; player groupChat "You got 1/4 of the civs bounty totaling $%2"}', player, _civkopfgeld, _self]) call broadcast;
		};
		...
		(format['if (player == %3)then{leavemybankmoneyalone = leavemybankmoneyalone + %2; player groupChat "You got 1/2 of the civs bounty totaling $%2"}', player, _civkopfgeld, _self]) call broadcast;
		};
	...
	sleep 1;
	(format ["[""KilledCiv"", %1, ""%2""] spawn Isse_AddCrimeLogEntry; if(%1 in civarray)then{if(!(""murder(5mins)"" in %1_reason))then{%1_reason = %1_reason + [""murder(5mins)""]}; kopfgeld_%1 = kopfgeld_%1 + 100000};", player, _killer]) call broadcast;
...
sleep 1;
(format ["[""KilledCop"", %1, ""%2""] spawn Isse_AddCrimeLogEntry; if(%1 in civarray)then{if(!(""murder(5mins)"" in %1_reason))then{%1_reason = %1_reason + [""murder(5mins)""]}; kopfgeld_%1 = kopfgeld_%1 + 100000};", player,  _killer]) call broadcast;
