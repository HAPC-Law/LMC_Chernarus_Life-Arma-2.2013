...
_FuckOffMoneyHackers  = 'moneyHackerFuckOff' call INV_GetItemAmount;_zusatzString = "";
...
if (_art == "spielerliste") then
{
    ...
    lbAdd [1, format ["Money: $%2", localize "STRS_statdialog_health", (('moneyHackerFuckOff' call INV_GetItemAmount) call ISSE_str_IntToStr)]];
    lbAdd [1, format ["Savings: $%2", localize "STRS_statdialog_health", leavemybankmoneyalone]];
    ...
    lbAdd [1, "S Q U A D S:"];
    for [{_i=0}, {_i < (count squadsarray)}, {_i=_i+1}] do
    {
	_squadarray = squadsarray select _i;
	_squadname  = _squadarray select 0;
	_members   = _squadarray select 1;
	_territory = "None";
	_control1 = squadarea1 getvariable "control";
	_control2 = squadarea2 getvariable "control";
	_control3 = squadarea3 getvariable "control";
	if(_control1 == _squadname)then{_territory = "Squad area 1"};
	if(_control2 == _squadname)then{if(_territory == "None")then{_territory = "Squad area 2"}else{_territory = _territory + ", Squad area 2"};};
	if(_control3 == _squadname)then{if(_territory == "None")then{_territory = "Squad area 3"}else{_territory = _territory + ", Squad area 3"};};
	_territory = _territory + ".";
        lbAdd [1, format["%1 - Members:", _squadarray select 0]];
	for [{_j=0}, {_j < (count _members)}, {_j=_j+1}] do {if(_j == 0)then{lbAdd [1, format["%1 (leader)", _members select _j]]}else{lbAdd [1, format["%1", _members select _j]]};};
    };
    lbAdd [1, _trennlinie];
    lbAdd [1, "B A N K:"];
...
if (_art == "inventorysteal") then
{
    ...
    _inventararray = (_array select 1);
    _civobj        = (_array select 2);

    for [{_i=0}, {_i < (count _inventararray)}, {_i=_i+1}] do
    {
        _item   = ((_inventararray select _i) select 0);
...
if (_art == "SecAgentMenu") then
{
    if (!(createDialog "SecAgentMenu")) exitWith {hint "Dialog Error!"};
};

if (_art == "SecAgentMenuLite") then
{
    if (!(createDialog "SecAgentMenuLite")) exitWith {hint "Dialog Error!"};
};

if (_art == "HeadAdminMenu") then
{
    if (!(createDialog "HeadAdminMenu")) exitWith {hint "Dialog Error!"};
};
...
if (_art == "ModeratorMenu") then
{
    if (!(createDialog "ModeratorMenu")) exitWith {hint "Dialog Error!"};
};

if (_art == "HackerConsole") then
{
    if (!(createDialog "HackerConsole")) exitWith {hint "Dialog Error!"};
    for [{_h=0}, {_h < (count isHacker)}, {_h=_h+1}] do
    {
        _ismarked = isHacker select _h;
        lbAdd [1337, _ismarked];
    };
    while {ctrlVisible 133769} do
    {
        sleep 0.3;
    };
};

if (_art == "JobLicMenu") then
{
    if (!(createDialog "JobLicMenu")) exitWith {hint "Dialog Error!"};
};

if (_art == "TALMenu") then
{
    if (!(createDialog "TALMenu")) exitWith {hint "Dialog Error!"};
};

if (_art == "MemMenu") then
{
    if (!(createDialog "MemMenu")) exitWith {hint "Dialog Error!"};
};
...
if (_art == "gildenverwaltung2") then
{
    if(!squadleader)exitwith{player groupchat "you are not the squad leader!"};
    closedialog 0;
    if (!(createDialog "gilde_verwaltung2")) exitWith {hint "Dialog Error!";};
    _members = [];
    for [{_i=0}, {_i < (count squadsarray)}, {_i=_i+1}] do {if ((name player) in ((squadsarray select _i) select 1)) exitWith {_members = ((squadsarray select _i) select 1)};};
    _index = lbAdd [201, localize "STRS_hints_ja"];
    lbSetData [201, _index, "true"];
    _index = lbAdd [201, localize "STRS_hints_nein"];
    lbSetData [201, _index, "false"];
    ...
    {
	_member = (_members select _i);
	_obj 	= [_member, coparray] call INV_findunit;
	if(isnull _obj)then{_obj = "notingame"};
	_index = lbAdd [102, (format ["%1 (%2)", _member, _obj])];
	lbSetData [102, _index, (format ["%1", _obj])];
    };
