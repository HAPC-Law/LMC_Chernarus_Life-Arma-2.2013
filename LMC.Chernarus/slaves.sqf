...
if (_FuckOffMoneyHackers <= slave_cost) exitWith {role groupChat localize "STRS_slave_nomoney"};
...
while {true} do 
	...
	for [{_i=0}, {_i < 60}, {_i=_i+1}] do 
		...
		if (player distance _slavename < 5 and _a1 == 0 and (call compile format ["arbeitergeld%1 > 0", _arbeiternummer])) then {_action = player addaction [localize "STRS_slave_moneyaction", "slaves.sqf",["moneyHackerFuckOff", _arbeiternummer]]; _a1 = 1;};															
