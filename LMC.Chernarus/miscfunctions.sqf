...
isprone = {if (animationstate player == "amovppnemrunsnonwnondf" or animationstate player == "amovppnemstpsnonwnondnon" or animationstate player == "amovppnemstpsraswrfldnon" or animationstate player == "amovppnemsprslowwrfldf") then {true} else {false};};
...
TeleToMainbaseFunc =
{
    if (not((vehicle player) == player)) then {player Action ["eject", vehicle player];};
    if (iscop) then {player setpos (getmarkerpos "respawn_west");} else {player setpos (getmarkerpos "hospitaltent");};
    player groupChat localize "STRS_verbotene_zone_teleport";
};

TurnMayorFunc = {if (_this) then {format["civ%1_mayor = 1;", rolenumber] call broadcast;} else {format["civ%1_mayor = 0;", rolenumber] call broadcast;};};

TurnChiefFunc = {if (_this) then {format["cop%1_chief = 1;", rolenumber] call broadcast;} else {format["cop%1_chief = 0;", rolenumber] call broadcast;};};

TurnBountyFunc = {if (_this) then {format["civ%1_bounty = 1;", rolenumber] call broadcast;} else {format["civ%1_bounty = 0;", rolenumber] call broadcast;};};

TurnWantedFunc = {if (_this) then {format["civ%1_wanted = 1;", rolenumber] call broadcast;} else {format["civ%1_wanted = 0;", rolenumber] call broadcast;};};
...
DialogSpielerListe =
{
    ...
    for [{_c=0}, {_c < (count playerstringarray)}, {_c=_c+1}] do
    {
        ...
        if (((_Fart == 0) or ((_Fart == 1) and (_c > civscount)) or ((_Fart == 2) and (_c < civscount))) and ((_Fingame) or (_Fspieler call ISSE_UnitExists))) then
        {
            ...
            if (_Fspieler == rolestring) then {_Feigenenum = _Flistlen;};
...
Isse_AddCrimeLogEntry =
{
    ...
    switch (_logart) do
    {
        ...
        case "KilledWorker": {_logtext = format[localize "STRS_functions_crimelog_msg13", _zusatzinfo];};
        ...
        case "slave": {_logtext = format[localize "STRS_functions_crimelog_msg11"];};
        case "Speed": {_logtext = format[localize "STRS_functions_crimelog_msg12"];};
        case "StoleVcl": {_logtext = format[localize "STRS_functions_crimelog_msg13", _zusatzinfo];};
        case "AttemptedStoleVcl": {_logtext = format[localize "STRS_functions_crimelog_msg14", _zusatzinfo];};
    };
...
Bomb_Vehicle =
{
    if (count _this > 1) then
    {
        ...
        {
            _obj = _this select 1;
            if (((getPos _obj) select 2) > 5) then {_obj setDamage 1;} else {"Bo_GBU12_LGB" createVehicle (_this select 0); _obj setDamage 1;};
        };
    }
    else
    {
    ...
    if (count _this > 2) then {format ["if ((random 100) < 10) then {hint format[localize ""STRS_vehiclebomb_public_explode"", %1]; [""Bomb"", %1] spawn Isse_AddCrimeLogEntry;};", player, longrolenumber] call broadcast;};
};

GetRandomObjectName =
{
    private ["_name"];
    if (isNil("GespawnteObjekteCounter")) then {GespawnteObjekteCounter = 0;};
...
SayDirectSpeach =
{
    ...
    if (not ((_text == "") or (_text == " "))) then
    {
        ...
        if (_art == "side") then {format ["%1 sideChat ""%2"";", player, _text] call broadcast;};
        if (_art == "global") then{format ["%1 globalChat ""%2"";", player, _text] call broadcast;};
    };
};
