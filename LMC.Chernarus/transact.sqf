...
if (INV_PLAYERLIST select _transfer == player) exitWith 
	...
	if (_FuckOffMoneyHackers < _transfermoney) then 
		...
		leavemybankmoneyalone = leavemybankmoneyalone + _transfermoney;
		['moneyHackerFuckOff', -(_transfermoney)] call INV_AddInventoryItem;
...
if (leavemybankmoneyalone < _steuern) then 
	...
	{
	if (_transfermoney <= 200000) then {
	leavemybankmoneyalone = leavemybankmoneyalone - _steuern;
	INV_SteuernGezahlt = (INV_SteuernGezahlt + (_steuern - _transfermoney));
	(format ["if (INV_ROLESTRING == ""%1"") then {leavemybankmoneyalone = leavemybankmoneyalone + %2; player groupChat format[localize ""STRS_bank_account_getmoney"", %2, %3];};", _targetString, (_transfermoney call ISSE_str_IntToStr), player]) call broadcast;
...
} else {hint "You can only transfer up to $200,000."};
...
if (leavemybankmoneyalone >= _withdraw) then 

	{

	leavemybankmoneyalone = leavemybankmoneyalone - _withdraw;
	['moneyHackerFuckOff', (_withdraw)] call INV_AddInventoryItem;
