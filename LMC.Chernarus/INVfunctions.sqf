...
INV_AddInventoryItem = {
	...
	if(_Famount > 30000000000000000) exitWith {
...
INV_AddItemStorage = {
	private ["_Fitem","_Fmenge","_Farrname","_Farr","_Farraynum","_Fanzahl","_Fextra","_maxGewicht","_curGewicht","_addGewicht","_i"];
	_Fitem      = _this select 0;
...
INV_AddProzent = {
	private ["_wert","_prozent","_result","_runden"];
...
INV_AddStorageToDialog = {
	private ["_c","_item","_anzahl","_infos","_KindOf","_CrctlID","_Findex","_Arrayname","_Array","_KindsOf"];
	_Arrayname = _this select 0;
...
private ["_mysquad","_squadarray","_squad","_members"];
_mysquad  = "";
...
for "_c" from 0 to (count squadsarray - 1) do 
	...
	if(name player in _members)then{_mysquad = _squad};
...
player globalchat "Terrorists have planted a bomb on the military HQ! The cops have 10 minutes to defuse the bomb!";
while {bombactive} do
{
