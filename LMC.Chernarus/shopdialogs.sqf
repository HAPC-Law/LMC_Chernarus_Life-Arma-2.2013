...
private ["_item","_preis","_preisOhneTax","_name","_index","_infos","_stock","_maxstock","_sellnumber","_demand","_itemsellarray","_i","_control","_mags","_menge","_j","_vehicle","_slider","_moneyanzeige","_cursel","_shop","_itembuyarray"];
...
ctrlSetText [101, format [localize "STRS_inv_shopdialog_itemshop", ('moneyHackerFuckOff' call INV_GetItemAmount)]];
//--------------------------------REFRESH-----------------------------------------
while {ctrlVisible 1015} do
{
    ...
    if (_cursel >= 0) then
    {
        ...
        if (_infos call INV_getitemType == "item") then {ctrlSetText [3, format ["Buy ($%1, %2kg)", _moneyanzeige, (_slider*(_infos call INV_getitemTypeKg))]];} else {ctrlSetText [3, format ["Buy ($%1)", _moneyanzeige]];};
    }
    else
    {
        ctrlSetText [3, "/"];
    ...
    if (_cursel >= 0) then
    {
        ...
        if (_infos call INV_getitemType == "item") then {ctrlSetText [103, format ["Sell ($%1, %2kg)", _moneyanzeige, (_slider*(_infos call INV_getitemTypeKg))]];} else {ctrlSetText [103, format ["Sell ($%1)", _moneyanzeige]];};
    }
    else
    {
        ctrlSetText [103, "/"];
