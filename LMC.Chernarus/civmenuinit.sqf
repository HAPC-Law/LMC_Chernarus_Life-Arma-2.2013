...
/*
    if((vehicle player distance CopPrisonAusgang > 10)) then {if(vehicle player distance quicktrans1 > 10) then {if(vehicle player distance quicktrans2 > 10) exitwith {hint "You are too far from the jail entrance or quick transport location.";};};};
	*/

	 if( ((vehicle player) distance CopPrisonAusgang < 10)) then {nearjail1 = 1} else {nearjail1 = 0;};
	 if( ((vehicle player) distance quicktrans1 < 10)) then {nearjail2 = 1} else {nearjail2 = 0};
	 if( ((vehicle player) distance quicktrans2 < 10)) then {nearjail3 = 1} else {nearjail3 = 0};
	 
if(nearJail1 == 1 || nearJail2 == 1 || nearJail3 == 1) then
{
cuffing = 0;
detach civmenuciv;
...
format ["if (player == %1) then {[""arrest"", %2, %3] execVM ""civmenu.sqf"";};", _civmenu_civ, _dauer, player] call broadcast;
	...
	} else {hint "You are too from the jail entrance or a jail transport location.";};
