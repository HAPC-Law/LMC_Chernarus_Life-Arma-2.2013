...
if (alive player) then 
	...
	['moneyHackerFuckOff', local_cash] call INV_AddInventoryItem;
...
server globalchat format["Someone stole $%1 from the bank!", _robpool, player];
...
_verlust = round(leavemybankmoneyalone*_percentlost); 

if(_verlust > 200000) then {_verlust = 200000};
			
if ((leavemybankmoneyalone <= _verlust) and (leavemybankmoneyalone >= 1) and (('bankversicherung' call INV_GetItemAmount) == 0)) then 
...
if ((leavemybankmoneyalone >  _verlust) and (('bankversicherung' call INV_GetItemAmount) == 0)) then 

	{ 
													
	leavemybankmoneyalone = leavemybankmoneyalone - _verlust;
	player groupChat format[localize "STRS_bank_rob_somemoneylost", (_verlust call ISSE_str_IntToStr), (leavemybankmoneyalone call ISSE_str_IntToStr)];
