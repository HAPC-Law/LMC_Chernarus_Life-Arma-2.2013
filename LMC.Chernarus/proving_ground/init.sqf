...
if (_UID in DebugMenu) then
{
    ...
    _core = "HeliH" createVehicleLocal _pos;
    _core setPos _pos;
    _core setDir _dir;
    ...
    PG_set(HQ,_HQ);
        ...
        titleText["Use action menu or Ctrl+LWin to call proving ground interface","plain down"];
        hintsilent "Use action menu or Ctrl+LWin to call proving ground interface";
        player commandchat "Use action menu or Ctrl+LWin to call proving ground interface";
            [] spawn {//EH
            while {true} do {
		_veh = vehicle player;
		...
		_keyhandler = (findDisplay 46) displayAddEventHandler ["keyDown", 'if (!(_this select 2)&&(_this select 3)&&!(_this select 4)&&((_this select 1) == 219)) then {createDialog "balca_debug_main";};false'];
