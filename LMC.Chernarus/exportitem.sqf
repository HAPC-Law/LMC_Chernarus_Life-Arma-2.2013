...
if (factoryitemexportactive == 1) exitwith {player groupchat "script already active"};
factoryitemexportactive = 1;

if(_stock >= _maxstock)exitwith{player groupchat "the shop has reached its maximum stock for this item"};
																			
['moneyHackerFuckOff', _preis] call INV_AddInventoryItem;
