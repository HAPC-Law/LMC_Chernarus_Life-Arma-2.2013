...
while {true} do 
	...
	if (!alive VIPtarget and alive player) exitWith 
		...
		leavemybankmoneyalone = leavemybankmoneyalone + 100000;
	...
	if(VIPtarget distance getmarkerpos "policebase" < 100) exitwith
		...
		if (iscop) then {leavemybankmoneyalone = (leavemybankmoneyalone + _copbonus); player sidechat format[""you received $%1 for the successful rescue of the VIP"", _copbonus];};
		" call broadcast;
		sleep 2;
